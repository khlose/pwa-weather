...
(function() {
  ...
  app.getForecast = function(key, label) {
    ...
    if('caches' in window){
        caches.match(url).then(function(response){
            if(response){
                console.log("response match return cache");
                response.json().then(function(jsonresp){
                    jsonresp.key = key;
                    jsonresp.label = label;
                    app.updateForecastCard(jsonresp);
                });
            }
        });
    }
  ...
  app.saveCities = function(){
      window.localforage.setItem('cities',app.selectedCities);
  };
    
  document.addEventListener('DOMContentLoaded',function(){
      window.localforage.getItem('cities').then(function(cityList){
          if(cityList){
...
=======
>>>>>>> c2a867a56cda6812bbff0f3d136318b8ffabf9d5
